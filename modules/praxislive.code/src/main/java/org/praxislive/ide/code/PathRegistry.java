...
public class PathRegistry {
    ...
    record Info(PraxisProject project, ClassPath classpath) {}
    ...
    public void register(PraxisProject project, FileObject root, FileObject sharedRoot) {
        registerImpl(project, root, sharedRoot);
    }
    ...
    private synchronized void registerImpl(PraxisProject project, FileObject root, FileObject sharedRoot) {

        ClassPath classpath = ClassPathSupport.createClassPath(root);

        if (sharedRoot != null) {
            Info sharedInfo = findInfo(sharedRoot);
            if (sharedInfo != null) {
        ...
        registry.put(root, new Info(project, classpath));

    }

    private synchronized void unregisterImpl(Project project, FileObject root) {
        Info info = registry.remove(root);
        if (info != null) {
