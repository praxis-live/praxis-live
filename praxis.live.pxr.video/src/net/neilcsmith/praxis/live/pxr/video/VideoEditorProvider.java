package net.neilcsmith.praxis.live.pxr.video;
...
public class VideoEditorProvider implements RootEditor.Provider {

    @Override
    public RootEditor createEditor(RootProxy model) {
        ComponentType type = model.getType();
        if (type.toString().equals("root:video")) {
            return new GraphEditor(model, "video");
        }
        return null;
    }
